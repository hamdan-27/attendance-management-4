...
if(isset($_POST['loginsubmit'])){
    ...
    $sql = "SELECT * FROM users WHERE user_email = '$user_email' AND user_password = '$user_password' AND user_role = 'teacher'";
    ...
    if($final > 0){
        ...
        if($final_check_user > 0){
            ...
            echo "<script>alert('Only teachers can login')</script>";
        } else {
            // Incorrect login credentials
            echo "<script>alert('Please enter correct details')</script>";
        }

        echo "<script>window.open('teacherlogin.php','_self')</script>";
    }
}
?>
